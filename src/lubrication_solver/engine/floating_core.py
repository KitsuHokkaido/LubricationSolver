...
class FloatingCore:
    ...
    def tau_2(self, h, dp_dx, tau_0):
        return self.tau_1(dp_dx, h, tau_0) + h * dp_dx

    def h_a(self, h, dp_dx, tau_0):
        #return ((h*dp_dx)**2 + 4*h*tau_0*dp_dx + 4 * tau_0**2 + 2*dp_dx*self._mu*(self._U1 - self._U2)) / (2 * dp_dx * (h*dp_dx + 2*tau_0))
        return (tau_0 - self.tau_1(h, dp_dx, tau_0)) / dp_dx

    def h_b(self, h, dp_dx, tau_0):
        #return ((h*dp_dx)**2 - 4 * tau_0**2 + 2 * dp_dx * self._mu*(self._U1 - self._U2)) / (2 * dp_dx*(h*dp_dx + 2 *tau_0))
    ...
    def _compute_dp_dx(self, q, h, tau_0) -> NDArray:
